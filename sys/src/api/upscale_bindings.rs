...
impl CreateContextUpscaleFlags {
    ...
    #[doc = "< A bit indicating if debug visualization is allowed. (memory consumption could increase)"]
    pub const DEBUG_VISUALIZATION: CreateContextUpscaleFlags = CreateContextUpscaleFlags(512);
}
...
impl DispatchUpscaleAutoreactiveFlags {
    #[doc = "< Apply tonemapping function prior to calculating reactivity."]
    pub const APPLY_TONEMAP: DispatchUpscaleAutoreactiveFlags = DispatchUpscaleAutoreactiveFlags(1);
    #[doc = "< Apply inverse tonemapping function prior to calculating reactivity."]
    pub const APPLY_INVERSETONEMAP: DispatchUpscaleAutoreactiveFlags =
        DispatchUpscaleAutoreactiveFlags(2);
    #[doc = "< Compare to a threshold value and only record reactivity above the threshold."]
    pub const APPLY_THRESHOLD: DispatchUpscaleAutoreactiveFlags =
        DispatchUpscaleAutoreactiveFlags(4);
    #[doc = "< When enabled use the max. component value, otherwise use the length to calculate reactivity."]
...
pub struct CreateContextDescUpscale {
    #[doc = "< Header descriptor, use type FFX_API_CREATE_CONTEXT_DESC_TYPE_UPSCALE."]
    pub header: CreateContextDescHeader,
    #[doc = "< Zero or a combination of values from FfxApiCreateContextUpscaleFlags."]
    pub flags: u32,
...
pub struct DispatchDescUpscale {
    #[doc = "< Header descriptor, use type FFX_API_DISPATCH_DESC_TYPE_UPSCALE."]
...
pub struct QueryDescUpscaleGetUpscaleRatioFromQualityMode {
    #[doc = "< Header descriptor, use type FFX_API_QUERY_DESC_TYPE_UPSCALE_GETUPSCALERATIOFROMQUALITYMODE."]
...
pub struct QueryDescUpscaleGetRenderResolutionFromQualityMode {
    #[doc = "< Header descriptor, use type FFX_API_QUERY_DESC_TYPE_UPSCALE_GETRENDERRESOLUTIONFROMQUALITYMODE."]
    pub header: QueryDescHeader,
...
pub struct QueryDescUpscaleGetJitterPhaseCount {
    #[doc = "< Header descriptor, use type FFX_API_QUERY_DESC_TYPE_UPSCALE_GETJITTERPHASECOUNT."]
    pub header: QueryDescHeader,
...
pub struct QueryDescUpscaleGetJitterOffset {
    #[doc = "< Header descriptor, use type FFX_API_QUERY_DESC_TYPE_UPSCALE_GETJITTEROFFSET."]
    pub header: QueryDescHeader,
...
pub struct DispatchDescUpscaleGenerateReactiveMask {
    #[doc = "< Header descriptor, use type FFX_API_DISPATCH_DESC_TYPE_UPSCALE_GENERATEREACTIVEMASK."]
    pub header: DispatchDescHeader,
    #[doc = "< The <c><i>FfxCommandList</i></c> to record FSRUPSCALE rendering commands into."]
    pub commandList: *mut ::std::os::raw::c_void,
    #[doc = "< A <c><i>FfxApiResource</i></c> containing the opaque only color buffer for the current frame (at render resolution)."]
    pub colorOpaqueOnly: Resource,
    #[doc = "< A <c><i>FfxApiResource</i></c> containing the opaque+translucent color buffer for the current frame (at render resolution)."]
    pub colorPreUpscale: Resource,
    #[doc = "< A <c><i>FfxApiResource</i></c> containing the surface to generate the reactive mask into."]
...
pub struct ConfigureDescUpscaleKeyValue {
    #[doc = "< Header descriptor, use type FFX_API_CONFIGURE_DESC_TYPE_UPSCALE_KEYVALUE."]
...
pub struct QueryDescUpscaleGetGPUMemoryUsage {
    #[doc = "< Header descriptor, use type FFX_API_QUERY_DESC_TYPE_UPSCALE_GPU_MEMORY_USAGE."]
    pub header: QueryDescHeader,
...
pub struct QueryDescUpscaleGetGPUMemoryUsageV2 {
    ...
    #[doc = "< App needs to fill out before Query() call."]
    pub maxUpscaleSize: Dimensions2D,
    #[doc = "< Zero or a combination of values from FfxApiCreateContextUpscaleFlags. App needs to fill out before Query() call."]
    pub flags: u32,
    #[doc = "< Output values by Query() call."]
    pub gpuMemoryUsageUpscaler: *mut EffectMemoryUsage,
}
...
#[non_exhaustive]
#[derive(Debug, Copy, Clone, Hash, PartialEq, Eq)]
pub enum QueryResourceIdentifiers {
    #[doc = "< Color buffer for the current frame (at render resolution)."]
    COLOR = 1,
    #[doc = "< 32bit depth values for the current frame (at render resolution)."]
    DEPTH = 2,
    #[doc = "< 2-dimensional motion vectors (at render resolution if FFX_FSR_ENABLE_DISPLAY_RESOLUTION_MOTION_VECTORS is not set)."]
    MV = 4,
    #[doc = "< A 1x1 texture containing exposure value or the FFX_UPSCALE_ENABLE_AUTO_EXPOSURE set at context creation."]
    EXPOSURE = 8,
    #[doc = "< An R8 UNORM texture at render resolution that controls the influence of pixel history on upscaling result."]
    REACTIVEMASK = 16,
    #[doc = "< An R8 UNORM texture at render resolution that controls the blending of color in the upscaling result."]
    TRANSPARENCYCOMPOSITION = 32,
}
#[repr(C)]
pub struct QueryDescUpscaleGetResourceRequirements {
    #[doc = "< Header descriptor, use type FFX_API_QUERY_DESC_TYPE_UPSCALE_GET_RESOURCE_REQUIREMENTS."]
    pub header: QueryDescHeader,
    #[doc = "< resources 64b bitfield, that given current context state, are required for effect correctness."]
    pub required_resources: u64,
    #[doc = "< resources 64b bitfield, that given current context state, will be consumed if provided, but are optional."]
    pub optional_resources: u64,
}
#[repr(C)]
pub struct CreateContextDescUpscaleVersion {
    #[doc = "< Header descriptor, use type FFX_API_CREATE_CONTEXT_DESC_TYPE_UPSCALE_VERSION."]
    pub header: CreateContextDescHeader,
...
pub const QUERY_DESC_TYPE_UPSCALE_GETUPSCALERATIOFROMQUALITYMODE: StructType_t =
    MAKE_EFFECT_SUB_ID(EFFECT_ID_UPSCALE, 0x02);

pub const QUERY_DESC_TYPE_UPSCALE_GETRENDERRESOLUTIONFROMQUALITYMODE: StructType_t =
    MAKE_EFFECT_SUB_ID(EFFECT_ID_UPSCALE, 0x03);

pub const QUERY_DESC_TYPE_UPSCALE_GETJITTERPHASECOUNT: StructType_t =
    MAKE_EFFECT_SUB_ID(EFFECT_ID_UPSCALE, 0x04);

pub const QUERY_DESC_TYPE_UPSCALE_GETJITTEROFFSET: StructType_t =
    MAKE_EFFECT_SUB_ID(EFFECT_ID_UPSCALE, 0x05);

pub const DISPATCH_DESC_TYPE_UPSCALE_GENERATEREACTIVEMASK: StructType_t =
    MAKE_EFFECT_SUB_ID(EFFECT_ID_UPSCALE, 0x06);

pub const CONFIGURE_DESC_TYPE_UPSCALE_KEYVALUE: StructType_t =
    MAKE_EFFECT_SUB_ID(EFFECT_ID_UPSCALE, 0x07);

pub const QUERY_DESC_TYPE_UPSCALE_GPU_MEMORY_USAGE: StructType_t =
    MAKE_EFFECT_SUB_ID(EFFECT_ID_UPSCALE, 0x08);

pub const QUERY_DESC_TYPE_UPSCALE_GPU_MEMORY_USAGE_V2: StructType_t =
    MAKE_EFFECT_SUB_ID(EFFECT_ID_UPSCALE, 0x09);

pub const QUERY_DESC_TYPE_UPSCALE_GET_RESOURCE_REQUIREMENTS: StructType_t =
    MAKE_EFFECT_SUB_ID(EFFECT_ID_UPSCALE, 0x0a);

pub const CREATE_CONTEXT_DESC_TYPE_UPSCALE_VERSION: StructType_t =
    MAKE_EFFECT_SUB_ID(EFFECT_ID_UPSCALE, 0x0b);
...
unsafe impl TaggedStructure for QueryDescUpscaleGetGPUMemoryUsageV2 {
    const TAG: StructType_t = QUERY_DESC_TYPE_UPSCALE_GPU_MEMORY_USAGE_V2;
    fn header(&self) -> &Header {
        &self.header
    }
    fn header_mut(&mut self) -> &mut Header {
...
unsafe impl TaggedStructure for QueryDescUpscaleGetResourceRequirements {
    const TAG: StructType_t = QUERY_DESC_TYPE_UPSCALE_GET_RESOURCE_REQUIREMENTS;
    fn header(&self) -> &Header {
        &self.header
    }
    fn header_mut(&mut self) -> &mut Header {
...
unsafe impl TaggedStructure for CreateContextDescUpscaleVersion {
    const TAG: StructType_t = CREATE_CONTEXT_DESC_TYPE_UPSCALE_VERSION;
    fn header(&self) -> &Header {
        &self.header
    }
    fn header_mut(&mut self) -> &mut Header {
